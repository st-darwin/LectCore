...
import { BookOpen, FileText, ArrowRight, Loader2, Sparkles, TrendingUp, Bell, Award, Calendar, CheckCircle2, Clock, Users, Plus, MessageSquare } from 'lucide-react';
...
const LecturerDashboard = () => {
  ...
  const updateClock = () => {
    const now = new Date();
    setCurrentTime(now.toLocaleDateString(undefined, { weekday: 'long', month: 'short', day: 'numeric' }));
  };
    ...
    try {
      ...
      const [coursesRes, materialsRes, announcementsRes] = await Promise.all([
        databases.listDocuments(
          ...
          [Query.equal('lecturerId', user.$id), Query.limit(4)]
        ),
        databases.listDocuments(
          appwriteConfig.databaseId,
        ...
        databases.listDocuments(
          ...
          [Query.equal('lecturerId', user.$id), Query.orderDesc('$createdAt'), Query.limit(3)]
        )
      ]);
  ...
  return (
    <div className="space-y-8 pb-12 animate-in fade-in duration-300">
      {/* Top Banner / Hero Segment */}
      <div className="relative overflow-hidden p-8 rounded-[2.5rem] bg-gradient-to-br from-indigo-900 via-indigo-800 to-slate-900 text-white shadow-2xl">
        <div className="absolute -right-12 -bottom-12 w-64 h-64 bg-indigo-500/20 rounded-full blur-3xl pointer-events-none" />
        <div className="absolute right-1/3 top-0 w-32 h-32 bg-emerald-500/10 rounded-full blur-2xl pointer-events-none" />
        
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-white/10 backdrop-blur-md border border-white/10 text-[11px] font-medium text-indigo-200">
            ...
            <p className="text-xs md:text-sm text-indigo-200/80 max-w-xl font-normal">
              Manage your assigned curricula, publish learning resources, and monitor student academic milestones seamlessly.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 shrink-0 flex-wrap">
            <button
              ...
              className="px-5 py-3 rounded-2xl bg-indigo-600/60 hover:bg-indigo-600 text-white text-xs font-bold border border-indigo-400/30 backdrop-blur-md transition-all cursor-pointer hover:scale-105 active:scale-95 flex items-center gap-2"
            >
            ...
            <button
              ...
              className="px-6 py-3 rounded-2xl bg-white text-indigo-900 hover:bg-indigo-50 text-xs font-bold shadow-lg transition-all cursor-pointer hover:scale-105 active:scale-95"
            >
              Manage Courses
            </button>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-6 rounded-[2.5rem] bg-white/80 backdrop-blur-xl border border-slate-200/70 shadow-[0_4px_20px_rgb(0,0,0,0.02)] flex items-center gap-4 group hover:border-indigo-200 transition-all">
        ...
        <div className="p-6 rounded-[2.5rem] bg-white/80 backdrop-blur-xl border border-slate-200/70 shadow-[0_4px_20px_rgb(0,0,0,0.02)] flex items-center gap-4 group hover:border-emerald-200 transition-all">
        ...
        <div className="p-6 rounded-[2.5rem] bg-white/80 backdrop-blur-xl border border-slate-200/70 shadow-[0_4px_20px_rgb(0,0,0,0.02)] flex items-center gap-4 group hover:border-amber-200 transition-all">
      ...
      {/* Main Content Sections: Split or Grid */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="space-y-4">
            ...
            {loading ? (
              <div className="flex items-center justify-center py-20 bg-white/40 rounded-[2.5rem] border border-slate-200/60">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {courses.map((course) => (
                  <div
                    ...
                    className="group p-6 rounded-[2.5rem] bg-gradient-to-b from-white/95 to-white/70 backdrop-blur-xl border border-slate-200/70 shadow-[0_4px_20px_rgb(0,0,0,0.02)] hover:shadow-xl hover:border-indigo-300 transition-all duration-300 cursor-pointer flex flex-col justify-between"
                  >
                    <div>
              ...
              <div className="text-center py-16 rounded-[2.5rem] bg-white/40 border border-dashed border-slate-200 backdrop-blur-sm">
                ...
                <p className="text-xs text-slate-400 mt-1">Courses assigned by administration will appear here automatically.</p>
              </div>
            )}
          </div>
          ...
          <div className="space-y-4">
            ...
            {announcements.length > 0 ? (
              <div className="space-y-3">
                {announcements.map((item) => (
                  <div 
                    key={item.$id}
                    onClick={() => navigate('/lecturer/announcements')}
                    className="p-5 rounded-2xl bg-white/80 border border-slate-200/70 hover:border-indigo-300 transition-all cursor-pointer flex items-center justify-between gap-4 shadow-xs"
                  >
                    <div className="space-y-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className={`px-2.5 py-0.5 rounded-full text-[9px] font-extrabold uppercase tracking-wider ${
                          item.priority === 'urgent' ? 'bg-rose-100 text-rose-700' : 'bg-indigo-50 text-indigo-600'
                        }`}>
                          {item.priority}
                        </span>
              ...
              <div className="p-8 rounded-[2.5rem] bg-white/40 border border-dashed border-slate-200 text-center">
        ...
        <div className="space-y-6">
          <div className="p-6 rounded-[2.5rem] bg-white/80 backdrop-blur-xl border border-slate-200/70 shadow-[0_4px_20px_rgb(0,0,0,0.02)] space-y-4">
            <h3 className="text-sm font-extrabold text-slate-900 uppercase tracking-tight">Quick Actions</h3>
            
            <div className="space-y-2.5">
              <button 
                onClick={() => navigate('/lecturer/announcements')}
                className="w-full p-3.5 rounded-2xl bg-slate-50 hover:bg-indigo-50 text-slate-700 hover:text-indigo-600 text-xs font-bold transition-all flex items-center justify-between group cursor-pointer border border-slate-200/60"
              >
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-xl bg-white text-indigo-600 flex items-center justify-center shadow-xs group-hover:bg-indigo-600 group-hover:text-white transition-colors">
                    <Plus size={15} />
                  </div>
              ...
              <div className="p-4 rounded-2xl bg-indigo-50/60 border border-indigo-100/60 space-y-2">
          ...
          <div className="p-6 rounded-[2.5rem] bg-gradient-to-br from-slate-900 to-indigo-950 text-white space-y-3 shadow-xl">
            <div className="flex items-center justify-between">
              <span className="px-2.5 py-0.5 rounded-full bg-indigo-500/30 border border-indigo-400/30 text-[10px] font-bold text-indigo-200 uppercase tracking-wider">
                System Notice
              </span>
